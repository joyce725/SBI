...
<script>
	...
	$(function() {
		...
		var channel_id = '<%= request.getParameter("channel_id") %>';
				...
				success : function(result) {
					...
					$.each(json_obj, function(i, item) {
						...
						$("#tbl_user").append('<tr><td><label>' + json_obj[i].user_name + '</label></td>' + 
								'<td>' + (json_obj[i].weight * 20) + '%</td>' +
								'<td>' + content[json_obj[i].user_authority - 1] + '</td>' +
								'<td><button name="' + json_obj[i].channel_id + '" value="'+ json_obj[i].user_id + '" class="btn-chkevaluate btn btn-wide btn-primary">查看評估內容</button></td>' + 
								'</tr>');
					});
					...
					$(".btn-chkevaluate").click(function(e){
						...
						var chan_id = $(this).prop("name");
						var user_id = $(this).prop("value");
						showEvaluate(chan_id, user_id);
		...
		function showEvaluate(chan_id, user_id) {
